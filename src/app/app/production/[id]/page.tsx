...
export default function KanbanBoardPage() {
    ...
    async function load() {
      ...
      const [{ data: b }, { data: cols }, { data: cardsRaw }] = await Promise.all([
        ...
        sb
          ...
          .select("*, work_orders(order_number, title, description, price, status, priority, due_date, created_at, customers(full_name, phone))")
          .eq("board_id", id)
          .order("position"),
      ])
  ...
  async function toggleUrgent(cardId: string, currentLabels: string[]) {
    ...
    const supabase = createClient()
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (supabase.from("production_cards") as any).update({ labels: newLabels }).eq("id", cardId)
  }
  ...
  async function createCard(colId: string) {
    ...
    }).select("*, work_orders(order_number, title, description, price, status, priority, due_date, created_at, customers(full_name, phone))").single()
    if (data) {
